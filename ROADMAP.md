...
| ~~P26~~ | ~~etcd baseline, failover, snapshot bench, latency breakdown~~ | **Done.** Reference Linux/XFS artifacts cover the etcd baseline, failover distribution, phase breakdown, write-path profile, cross-node curve, and a three-run 1 GiB snapshot transfer. |
...
| ~~P30~~ | ~~Operator surface and snapshot benchmark~~ | **Done.** `keel-admin` drives add-learner, caught-up promotion, remove, leader transfer and manual snapshot through the single-threaded host loop. A four-process test covers learner → voter → removed. `keel-bench snapshot` and `scripts/snapshot-bench.sh` measure checkpoint stall and real-process transfer; the committed 1 GiB Reference result reports a 17 ms median write stall. |
