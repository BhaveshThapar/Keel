...
Transfer the repository **after the v3.0.4 tag exists and
`scripts/release-checklist.sh v3.0.4` passes at that tag**. The code is then
...
Git bundle, or `rsync` excluding `target/`), check out `v3.0.4`, then run:

```sh
git status --short
git describe --exact-match --tags HEAD
scripts/release-checklist.sh v3.0.4
